...
namespace embree
{
  ...
  public:
    ...
    void shade(const Ray& ray, const Medium& currentMedium, const DifferentialGeometry& dg, CompositedBRDF& brdfs) const
	{
	  ...
	  if (Kd)
	  {
		  Color4  Farbe = Kd->get(ds*dg.st + s0);

		  Color4 Magenta = Color4(1.0f, 0.0f, 1.0f, 1.0f);
		  Color4 Weiss   = Color4(1.0f, 1.0f, 1.0f, 1.0f);

		  // Farbe Magenta (RGB:255,0,255) als Transparenz betrachten
		  if (Farbe == Magenta)
			  brdfs.add(NEW_BRDF(Transmission)(Weiss));
		  else
			  brdfs.add(NEW_BRDF(Lambertian)(Farbe));
	  }
